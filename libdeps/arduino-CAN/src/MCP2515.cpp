...
int8_t MCP2515Class::setMaskAndFilter(uint16_t *mask, uint16_t *filter, uint8_t f_len) {
  // config mode
  writeRegister(REG_CANCTRL, 0x80);
  if (readRegister(REG_CANCTRL) != 0x80) {
    return 0;
  ...
  for (; i < 6; i++) {
      writeRegister(REG_RXFnSIDH(i), 0);
      writeRegister(REG_RXFnSIDL(i), 0);
      writeRegister(REG_RXFnEID8(i), 0);
      writeRegister(REG_RXFnEID0(i), 0);
  }
...
//int MCP2515Class::filter(int id, int filter) { return 1; }
...
int MCP2515Class::sleep() {
  // Disable interrupt for wakeup
  modifyRegister(REG_CANINTE, B01000000, 0x00);
  // Clear
  modifyRegister(REG_CANINTF, B01000000, 0x00);
...
int MCP2515Class::wakeup() {
  ...
  modifyRegister(REG_CANINTE, B01000000, B01000000);
  // Trigger the interrupt for wakening up
  modifyRegister(REG_CANINTF, B01000000, B01000000);
  modifyRegister(REG_CANINTF, B01000000, 0x00);
  delay(30);
  // Normal Mode
  modifyRegister(REG_CANCTRL, B11100000, B00000000);
  delay(30);
  return 1;
}
